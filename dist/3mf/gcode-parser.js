...
function parseWords(command, line) {
    const words = new Map();
...
function directedSweep(start, end, clockwise, fullCircle) {
    if (fullCircle)
        return clockwise ? -TAU : TAU;
    if (clockwise) {
...
function angleOnSweep(angle, start, sweep) {
    if (Math.abs(sweep) >= TAU - 1e-12)
        return true;
    if (sweep > 0)
        return normalizePositive(angle - start) <= sweep + 1e-12;
    return normalizePositive(start - angle) <= -sweep + 1e-12;
}
function arcGeometry(start, end, i, j, clockwise, fullCircle, line) {
...
async function* boundedLines(stream, limits, signal, deadlineMs) {
...
export async function parseGcodeStream(stream, options = {}) {
    ...
    try {
        for await (const rawLine of boundedLines(stream, limits, options.signal, options.deadlineMs)) {
            ...
            if (trimmed.startsWith(";")) {
                ...
                if (width) {
                    state.widthMm = finiteNumber(width[1], "LINE_WIDTH", lineCount) ?? null;
                    continue;
                }
                const height = /^LAYER_HEIGHT:\s*(\S+)/i.exec(comment);
                if (height) {
                    state.heightMm = finiteNumber(height[1], "LAYER_HEIGHT", lineCount) ?? null;
                    continue;
                }
                const zHeight = /^Z_HEIGHT:\s*(\S+)/i.exec(comment);
                if (zHeight) {
                    state.zHeight = finiteNumber(zHeight[1], "Z_HEIGHT", lineCount) ?? null;
                    continue;
                }
                ...
                if (/^WIPE_START\b/i.test(comment)) {
                    state.wipe = true;
                    continue;
                }
                if (/^WIPE_END\b/i.test(comment)) {
                    state.wipe = false;
                    continue;
                }
                ...
                if (header) {
                    ...
                    if ((key.includes("slicer") || key === "generated by") && !slicerVersion)
                        slicerVersion = header[2].trim();
                }
                continue;
            }
            ...
            if (letter === "T") {
                if (Number.isInteger(code) && code >= 0)
                    state.toolIndex = code;
                continue;
            }
            if (letter === "M") {
                if (code === 82)
                    state.eAbsolute = true;
                else if (code === 83)
                    state.eAbsolute = false;
                continue;
            }
            if (letter !== "G")
                continue;
            if (code === 90) {
                state.xyzAbsolute = true;
                continue;
            }
            if (code === 91) {
                state.xyzAbsolute = false;
                continue;
            }
            if (code === 17) {
                state.plane = "XY";
            ...
            for (const [axis, key] of [["x", "X"], ["y", "Y"], ["z", "Z"]]) {
                const value = words.get(key);
                if (value !== undefined)
                    end[axis] = state.xyzAbsolute ? value : start[axis] + value;
            ...
            state.position = end;
            state.e = nextE;
            if (spatial || deltaE !== 0 || code === 2 || code === 3)
                await emit(primitive);
        }
    }
    catch (error) {
        stream.destroy();
