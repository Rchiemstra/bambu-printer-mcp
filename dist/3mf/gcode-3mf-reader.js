...
async function boundedBuffer(stream, maxBytes, signal, deadlineMs) {
    const chunks = [];
    let total = 0;
    try {
...
export class SecureGcode3mfReader {
    constructor(sourcePath, plateIndex, limits, entries) {
    ...
    static async open(sourcePath, plateIndex = 0, options = {}) {
        ...
        try {
            while (true) {
                ...
                if (count > limits.maxEntries)
                    throw new SupportToolError("ARCHIVE_ENTRY_LIMIT_EXCEEDED", "3MF archive has too many entries.");
    ...
    async openEntryStream(name, signal, deadlineMs) {
        const wanted = name.toLowerCase();
        if (!this.entries.has(wanted))
            throw new SupportToolError("ZIP_ENTRY_NOT_FOUND", `3MF entry not found: ${name}.`);
        ...
        while (true) {
            ...
            if (safeEntryName(entry.fileName).toLowerCase() !== wanted)
                continue;
    ...
    openGcodeStream(signal, deadlineMs) {
        return this.openEntryStream(this.gcodeEntryName, signal, deadlineMs);
    }
    async readEntryBuffer(name, signal, deadlineMs) {
    ...
    async readOptionalEntryBuffer(name, signal, deadlineMs) {
        return this.hasEntry(name) ? this.readEntryBuffer(name, signal, deadlineMs) : null;
    }
    async verifyMd5IfPresent(signal, deadlineMs) {
        const md5Name = `${this.gcodeEntryName}.md5`;
        if (!this.hasEntry(md5Name))
            return;
        ...
        if (!expectedMatch)
            throw new SupportToolError("MALFORMED_GCODE_CHECKSUM", "Plate G-code MD5 sidecar is malformed.");
