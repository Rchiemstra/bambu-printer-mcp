...
import { MeshSupportAnalyzer, parse3mfTriangles } from "./mesh-analysis.js";
import { SupportMetricsCollector } from "./support-metrics.js";
...
function validateInput(input) {
    ...
    if (input.sensitive_regions !== undefined && !Array.isArray(input.sensitive_regions))
        throw new SupportToolError("INVALID_SENSITIVE_REGIONS", "sensitive_regions must be an array.");
    return { plateIndex, timeoutMs, meshAnalysis: input.mesh_analysis ?? true };
}
function parseJson(buffer, label, warnings) {
    if (!buffer)
        return null;
    try {
...
function firstSetting(config, key) {
    if (!config)
        return undefined;
...
function filamentMetadata(config) {
...
export async function analyze3mfSupports(input, options = {}) {
