...
import { createCamera, projectPoint } from "./camera.js";
const COLORS = {
...
function visible(primitive, input) {
    const layer = primitive.layerIndex;
    if (input.layer_start !== undefined && (layer === null || layer < input.layer_start))
        return false;
    if (input.layer_end !== undefined && (layer === null || layer > input.layer_end))
        return false;
    if (primitive.category === "support_body" || primitive.category === "support_transition")
        return input.show_support && primitive.positiveEDeltaMm > 0;
    if (primitive.category === "support_interface")
        return input.show_support_interface && primitive.positiveEDeltaMm > 0;
    if (primitive.category === "travel")
        return input.show_travel;
    return input.show_model && primitive.positiveEDeltaMm > 0;
}
function include(target, incoming, expand) {
    ...
    if (!target)
        return value;
    target.min.x = Math.min(target.min.x, value.min.x);
    target.min.y = Math.min(target.min.y, value.min.y);
    target.min.z = Math.min(target.min.z, value.min.z);
    target.max.x = Math.max(target.max.x, value.max.x);
    target.max.y = Math.max(target.max.y, value.max.y);
    target.max.z = Math.max(target.max.z, value.max.z);
    return target;
}
function colorFromHex(hex) {
    const match = /^#([0-9a-f]{6})([0-9a-f]{2})?$/i.exec(hex);
    if (!match)
        throw new SupportToolError("INVALID_BACKGROUND_COLOR", "background_color must be #RRGGBB or #RRGGBBAA.");
...
function primitivePoints(primitive, pixelsPerMm) {
    if (primitive.kind === "line" || !primitive.center || primitive.sweepRadians === undefined)
...
function rasterLine(data, depth, width, height, a, b, radius, color) {
    ...
    for (let y = minY; y <= maxY; y++)
        for (let x = minX; x <= maxX; x++) {
            ...
            if (coverage <= 0)
                continue;
            const z = a.depth + (b.depth - a.depth) * t, index = y * width + x;
            if (z < depth[index] - 1e-12)
                continue;
            depth[index] = z;
            ...
            data[offset + 3] = Math.round((alpha + (data[offset + 3] / 255) * inv) * 255);
        }
}
export async function renderPreviewInWorker(input) {
    ...
    const first = parseGcodeStream(await reader.openGcodeStream(undefined, input.deadline_ms), { deadlineMs: input.deadline_ms, onPrimitive: (primitive) => {
            collector.accept(primitive);
            if (!visible(primitive, input))
                return;
            roles.add(primitive.role ?? primitive.category);
            bounds = include(bounds, primitive.bounds, (primitive.widthMm ?? 0) / 2);
        } });
    const parsed = await first;
    if (!bounds)
        throw new SupportToolError("NO_VISIBLE_TOOLPATHS", "No toolpaths are visible with the requested role and layer filters.");
    const metrics = collector.finish(parsed);
    const camera = createCamera(input.view, bounds, input.width, input.height);
    const png = new PNG({ width: input.width, height: input.height });
    const bg = colorFromHex(input.background_color);
    for (let i = 0; i < input.width * input.height; i++) {
        const o = i * 4;
        png.data[o] = bg[0];
        png.data[o + 1] = bg[1];
        png.data[o + 2] = bg[2];
        png.data[o + 3] = input.background === "transparent" ? 0 : bg[3];
    }
    const depth = new Float64Array(input.width * input.height);
    ...
    await parseGcodeStream(await reader.openGcodeStream(undefined, input.deadline_ms), { deadlineMs: input.deadline_ms, onPrimitive: (primitive) => {
            if (!visible(primitive, input))
                return;
            if ((++commands & 0x1fff) === 0)
                throwIfCancelled(undefined, input.deadline_ms);
            const points = primitivePoints(primitive, camera.pixels_per_mm);
            const radius = primitive.category === "travel" ? 0.55 : Math.max(0.65, (primitive.widthMm ?? 0.4) * camera.pixels_per_mm / 2);
            const color = COLORS[primitive.category] ?? COLORS.unknown;
            for (let i = 1; i < points.length; i++)
                rasterLine(png.data, depth, input.width, input.height, projectPoint(points[i - 1], camera, input.width, input.height), projectPoint(points[i], camera, input.width, input.height), radius, color);
        } });
    ...
    if (encoded.length > 8 * 1024 * 1024)
        throw new SupportToolError("RENDER_LIMIT_EXCEEDED", "Encoded PNG exceeds the 8 MiB limit.");
