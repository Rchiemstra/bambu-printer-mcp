...
type Matrix = number[];
type Triangle = { a: Vec3; b: Vec3; c: Vec3; min: Vec3; max: Vec3 };
...
export interface MeshAnalysisResult {
  ...
  mesh_warnings: Array<{ code: string; severity: "warning"; message: string; affects?: string[] }>;
}
...
function multiply(a: Matrix, b: Matrix): Matrix {
  ...
  for (let r=0;r<4;r++) for (let c=0;c<4;c++) for (let k=0;k<4;k++) out[r][c] += a4[r][k] * b4[k][c];
...
function transform(point: Vec3, matrix: Matrix, scale: number): Vec3 {
  return {
...
function unitScale(unit: string | undefined): number {
  const scales: Record<string, number> = { micron: 0.001, millimeter: 1, centimeter: 10, inch: 25.4, foot: 304.8, meter: 1000 };
  return scales[(unit ?? "millimeter").toLowerCase()] ?? 1;
}

function children(node: any, suffix: string): any[] {
...
export async function parse3mfTriangles(modelXml: Buffer, plateObjectIds?: Set<number>): Promise<Triangle[]> {
  let parsed: any;
  try {
  ...
  for (const object of children(resources, "object")) {
    ...
    if (Number.isInteger(id)) objects.set(id, object);
  }
  const output: Triangle[] = [];
  const visit = (objectId: number, matrix: Matrix, stack: Set<number>): void => {
    if (stack.has(objectId)) throw new SupportToolError("CYCLIC_3MF_COMPONENT", `3MF component cycle includes object ${objectId}.`);
    const object = objects.get(objectId);
    if (!object) throw new SupportToolError("MISSING_3MF_OBJECT", `3MF references missing object ${objectId}.`);
    ...
    if (mesh) {
      ...
      const vertices = children(verticesNode, "vertex").map((vertex) => {
        ...
        if (![point.x,point.y,point.z].every(Number.isFinite)) throw new SupportToolError("MALFORMED_3MF_MESH", "3MF vertex contains a non-finite coordinate.");
  ...
  for (const item of children(build, "item")) {
    ...
    if (plateObjectIds && plateObjectIds.size > 0 && !plateObjectIds.has(id)) continue;
...
function pointTriangleDistance(p: Vec3, t: Triangle): number {
  ...
  const d1=dot(ab,ap), d2=dot(ac,ap); if(d1<=0&&d2<=0)return distance(p,t.a);
  const bp=sub(p,t.b), d3=dot(ab,bp), d4=dot(ac,bp); if(d3>=0&&d4<=d3)return distance(p,t.b);
  const vc=d1*d4-d3*d2; if(vc<=0&&d1>=0&&d3<=0){const v=d1/(d1-d3);return distance(p,add(t.a,mul(ab,v)));}
  const cp=sub(p,t.c), d5=dot(ab,cp), d6=dot(ac,cp); if(d6>=0&&d5<=d6)return distance(p,t.c);
  const vb=d5*d2-d1*d6; if(vb<=0&&d2>=0&&d6<=0){const w=d2/(d2-d6);return distance(p,add(t.a,mul(ac,w)));}
  const va=d3*d6-d5*d4; if(va<=0&&(d4-d3)>=0&&(d5-d6)>=0){const w=(d4-d3)/((d4-d3)+(d5-d6));return distance(p,add(t.b,mul(sub(t.c,t.b),w)));}
  const denom=1/(va+vb+vc),v=vb*denom,w=vc*denom;return distance(p,add(t.a,add(mul(ab,v),mul(ac,w))));
}

function regionBounds(region: SensitiveRegion): { min: Vec3; max: Vec3 } {
  if (region.kind === "box") {
    const size = region.size;
    if (!size || [size.x,size.y,size.z].some((value)=>!Number.isFinite(value)||value<=0)) throw new SupportToolError("INVALID_SENSITIVE_REGION", `Box annotation ${region.id} requires a positive size.`);
  ...
  if (!(radius&&radius>0&&height&&height>0)) throw new SupportToolError("INVALID_SENSITIVE_REGION", `Cylinder annotation ${region.id} requires positive radius_mm and height_mm.`);
...
function intersects(a:{min:Vec3;max:Vec3},b:{min:Vec3;max:Vec3},expand=0):boolean{return a.min.x-expand<=b.max.x&&a.max.x+expand>=b.min.x&&a.min.y-expand<=b.max.y&&a.max.y+expand>=b.min.y&&a.min.z-expand<=b.max.z&&a.max.z+expand>=b.min.z;}

export class MeshSupportAnalyzer {
  private readonly grid = new Map<string, number[]>();
  private readonly globalTriangles: number[] = [];
  private readonly gridSizeMm = 10;
  private readonly contacts = new Map<string,{sum:Vec3;count:number;area:number}>();
  private readonly blocked = new Map<string,number>();
  private minimumClearance = Number.POSITIVE_INFINITY;
  private readonly annotationBounds: Array<{region:SensitiveRegion;bounds:{min:Vec3;max:Vec3}}>;

  constructor(private readonly triangles: Triangle[], private readonly annotations: SensitiveRegion[] = []) {
    ...
    for (let index = 0; index < triangles.length; index += 1) {
      ...
      if(cells>4096||references+cells>2_000_000){this.globalTriangles.push(index);continue;}
      for(let x=minX;x<=maxX;x++)for(let y=minY;y<=maxY;y++)for(let z=minZ;z<=maxZ;z++){
        const key=`${x},${y},${z}`,bucket=this.grid.get(key);if(bucket)bucket.push(index);else this.grid.set(key,[index]);references+=1;
      }
    }
  }

  private nearby(point:Vec3):Triangle[]{
    const cx=Math.floor(point.x/this.gridSizeMm),cy=Math.floor(point.y/this.gridSizeMm),cz=Math.floor(point.z/this.gridSizeMm);const indices=new Set<number>(this.globalTriangles);
    for(let x=cx-1;x<=cx+1;x++)for(let y=cy-1;y<=cy+1;y++)for(let z=cz-1;z<=cz+1;z++)for(const index of this.grid.get(`${x},${y},${z}`)??[])indices.add(index);
  ...
  accept(primitive: MotionPrimitive): void {
    ...
    for (const item of this.annotationBounds) if(intersects(primitive.bounds,item.bounds,radius)) this.blocked.set(item.region.id,(this.blocked.get(item.region.id)??0)+1);
    ...
    for(const sample of samples){
      for(const tri of this.nearby(sample)){
        if(sample.x<tri.min.x-5||sample.x>tri.max.x+5||sample.y<tri.min.y-5||sample.y>tri.max.y+5||sample.z<tri.min.z-5||sample.z>tri.max.z+5)continue;
        const d=pointTriangleDistance(sample,tri); if(d<closest){closest=d;contactPoint=sample;}
      }
    }
    if(Number.isFinite(closest)) this.minimumClearance=Math.min(this.minimumClearance,Math.max(0,closest-radius));
    if(contactPoint&&closest<=radius+0.2){
      ...
      old.sum=add(old.sum,contactPoint);old.count+=1;
      if(primitive.category==="support_interface")old.area+=primitive.pathLengthMm*(primitive.widthMm??0.4);
      this.contacts.set(key,old);
    }
  }

  finish(): MeshAnalysisResult {
    const contact_regions=[...this.contacts.entries()].map(([id,value])=>({id:`contact-${id}`,center:mul(value.sum,1/value.count),estimated_area_mm2:Number(value.area.toFixed(6)),sample_count:value.count,confidence:(value.count>=3?"medium":"low") as "medium"|"low"}));
    const blocked_openings=this.annotations.map((region)=>({annotation_id:region.id,semantic:region.semantic??null,support_segment_count:this.blocked.get(region.id)??0,confidence:"medium" as const,heuristic:"support_envelope_intersects_annotation" as const})).filter((item)=>item.support_segment_count>0);
