...
import { renderPreviewInWorker, type RenderWorkerInput } from "./software-renderer.js";
import { asSupportToolError } from "../support/support-error.js";

if (!parentPort) throw new Error("render-worker must run in a worker thread");

try {
  const result = await renderPreviewInWorker(workerData as RenderWorkerInput);
